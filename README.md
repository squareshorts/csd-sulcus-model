...
This repository contains a reproducible cortical spreading depolarization (CSD) modeling workflow for studying sulcal slowing in folded cortex. The project supports isotropic scalar coupling reductions, orientation-aware tensor transport, an explicit electromagnetic cancellation term for opposed sulcal-bank dipole geometry, a physiology-constrained tensor extension tied to conservative cortical anisotropy priors, and a reduced biophysical potassium-buffer SD model for checking whether the scalar-versus-tensor ordering survives beyond Barkley kinetics.
...
- `scripts/run_surface_representative.py`: cortical-surface representative scaffold with anisotropy, explicit sulcal electromagnetic cancellation, and low-order vascular feedback
...
The prep script also derives a tangential preferred-axis field from the sulcal-depth gradient so the anisotropic transport term has an anatomically grounded surface direction field. The surface solver then combines sulcal depth with cortical-wall tilt relative to a shallow gyral reference normal to build an explicit electromagnetic cancellation field, representing the opposed, near-colinear dipole geometry of pyramidal populations on facing sulcal banks.
