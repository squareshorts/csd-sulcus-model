...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig_mechanistic_atlas_patch_qc_notitles.png}
\caption{Atlas-based sulcal-versus-flatter patch sanity check used as a geometry-specific supplement to the main mechanistic results. Left, the fsaverage6 left-hemisphere midthickness surface with the selected sulcal and flatter matched patches overlaid on the normalized sulcal-depth map. Right, the stimulus-to-downstream propagation speed for each patch with dipole alignment disabled or enabled. The larger slowdown on the sulcal patch supports the sign of the atlas sanity check reported in the main manuscript, but this figure remains supplementary because it is a single atlas-based check rather than a full subject-specific validation study.}
